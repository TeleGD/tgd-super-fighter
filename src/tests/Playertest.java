...
public class Playertest extends Movable implements Rectangle{
	...
	public void verticalMove() {
		...
		posjump = false;
		for (int i = 0; i < tests.World2.getPlateforms().size(); i++) {
			if ((tests.World2.getPlateforms().get(i).collPlayer(this)) && (!downPress)) {
				this.y = tests.World2.getPlateforms().get(i).getY() - this.height;
				this.accelY = 0;
				this.speedY = 0;
				colplat = true;
				vertcolthis = true;
