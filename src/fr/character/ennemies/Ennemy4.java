...
//	private double x, y, xorigine, yorigine;
	...
	public Ennemy4(int x, int y, double xorigine, double yorigine) {
		...
		try {
			img=new Image("Images/Images ennemis/Ennemytourelles/mage.png");
	...
	public void update(GameContainer container, StateBasedGame game, int delta) throws SlickException {
		...
		if(compt>50){
			compt=0;
			this.xtir=(fr.main.World.getPlayer().getnewX()-(x+this.width/2));
			this.ytir=(fr.main.World.getPlayer().getnewY()-(y+height/4));
			norm=Math.sqrt(xtir*xtir+ytir*ytir);
			xtir=0.3*xtir/norm;
			ytir=0.3*ytir/norm;
			fr.main.World.createStraightProjectil((this.x+this.width/2),this.y+this.height/4, xtir, ytir);
		}
